...
[![Rust](https://github.com/ekoDB/hnswlib-rs/actions/workflows/rust.yml/badge.svg)](https://github.com/ekoDB/hnswlib-rs/actions/workflows/rust.yml)

## About this fork

This is the [ekoDB](https://ekodb.io)-maintained fork of [jean-pierreBoth/hnswlib-rs](https://github.com/jean-pierreBoth/hnswlib-rs). The crate provides the HNSW approximate nearest neighbor index that powers vector search in the ekoDB database, with [anndists](https://github.com/ekoDB/anndists) (also forked) supplying the distance kernels.

**Why a fork.** The crate ships inside a database, so we need direct control over the exact code in the stack: the ability to pin and audit every revision, land fixes on our own schedule, and add capabilities the product needs without waiting on upstream releases. Fork additions so far include soft-deletion with neighbor repair, bulk insert methods, SIMD enabled by default, and removal of the unmaintained bincode dependency; every change runs through CI (rustfmt, clippy `-D warnings`, tests).
...
**Development.** Run `make setup` once (toolchain target, cargo tools, git hooks), then `make help` lists all targets; `make lint` and `make test` run exactly what CI runs, and `make install-hooks` installs a pre-commit hook that runs both. On Apple Silicon, `make test-x86` runs the test suite under Rosetta (note that real AVX2 SIMD coverage comes from CI on x86_64 hardware).
...
- Dump and reload functions (_See module hnswio_) to store the data and the graph once it is built. These facilities rely partly on Serde so T needs to implement Serialize and Deserialized as derived by Serde.
...
- A flattening conversion of the Hnsw structure to keep only neighborhood relationships between points (without their internal data) internal to the Hnsw structure (_see module flatten.rs, FlatPoint and FlatNeighborhood_). It is thus possible to keep some topology information with low memory usage.
...
Distances are provided by the crate [anndists](https://github.com/jean-pierreBoth/anndists), see _Building_.
...
The nearest neighbour of the point is searched in lookup tables from the upper level to the level just above its layer (**_l_**), so we should arrive near the new point at its level at a relatively low cost. Then the **_max_nb_connection_** nearest neighbours are searched in neighbours of neighbours table (with a reverse updating of tables) recursively from its layer **_l_** down to the most populated level 0.

The parameter of the exponential law to sample point levels is set to `ln(max_nb_connection)/scale`.
By default _scale_ is set to 1. It is possible to reduce the _scale_ parameter and thus reduce the number of levels used (See Hnsw::modify*level_scale) without increasing max_nb_connection.  
This often provide better recalls without increasing \_max_nb_connection* and thus spare memory usage. (See examples)
...
- ef*arg (in search methods)  
   This parameter controls the width of the search in the lowest level, it must be greater than number of neighbours asked but can be less than \*\*\_ef_construction*\*\*.
